# Standalone Mask2Former config for leaf instance segmentation
# Dataset settings
dataset_type = 'CocoDataset'
data_root = 'data/instance/'
...
val_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='annotations/instances_val2017_no_lesion.json',
...
val_evaluator = dict(
    type='CocoMetric',
    ann_file=data_root + 'annotations/instances_val2017_no_lesion.json',
    metric=['bbox', 'segm'],
    classwise=True)
...
model = dict(
    type='Mask2Former',   # keep this detector type
    backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4,
...
param_scheduler = dict(
    ...
    gamma=0.1,
    milestones=[20000])  # learning rate decay milestone

train_cfg = dict(
    type='IterBasedTrainLoop', max_iters=5000, val_interval=1000)
...
resume = False
launcher = 'pytorch'
work_dir = './work_dirs/mask2former_leaf'
