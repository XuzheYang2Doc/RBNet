...
class BiformerDecoder(DetrTransformerDecoder):
    def _init_layers(self) -> None:
        """Initialize decoder layers."""
        self.layers = ModuleList([
            BiformerTransformerDecoderLayer(256)
            for _ in range(4)
        ])
        self.embed_dims = 256
        self.post_norm = build_norm_layer(self.post_norm_cfg,
...
class BiformerTransformerDecoderLayer(DetrTransformerDecoderLayer):
    """Implements decoder layer with Biformer cross attention."""
    ...
    def __init__(self,
        ...
        # cross-attn with Biformer
        self.cross_attn1 = BiLevelRoutingAttention(
            dim=dim,
            num_heads=num_heads,
            n_win=n_win,
            qk_dim=qk_dim,
        ...
        self.norms1 = nn.ModuleList([nn.LayerNorm(dim) for _ in range(3)])

        # self-attn (standard MHA)
        self.self_attn1 = nn.MultiheadAttention(embed_dim=dim, num_heads=num_heads, batch_first=True)
        ...
        # projections for flatten input to 2D map for Biformer
        self.proj_in1 = nn.Linear(dim, dim)
        self.proj_out1 = nn.Linear(dim, dim)
    ...
    def forward(self,
        ...
        """
        Args:
            query (Tensor): (bs, num_queries, dim)
        """
