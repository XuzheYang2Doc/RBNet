...
class MobileViT(BaseDecodeHead):
    def __init__(self, img_size, features_list, d_list, transformer_depth, expansion, **kwargs):
        super(MobileViT, self).__init__(**kwargs)
        ...
        self.decoder4 = DoubleConv(features_list[9] + features_list[7], features_list[7])
        self.decoder3 = DoubleConv(features_list[7] + features_list[5], features_list[5])
        self.decoder2 = DoubleConv(features_list[5] + features_list[3], features_list[3])
        self.decoder1 = DoubleConv(features_list[3] + 3, features_list[1])

        self.head = nn.Sequential(
            nn.Conv2d(features_list[1], 2, kernel_size=1, stride=1),
        )
    ...
    def forward(self, x):
        ...
        x4_in = F.interpolate(x4, (x3.shape[2], x3.shape[3]), mode='bilinear', align_corners=True)
        x3_de = self.decoder4(torch.cat([x3, x4_in], dim=1))
        x3_in = F.interpolate(x3_de, (x2.shape[2], x2.shape[3]), mode='bilinear', align_corners=True)
        x2_de = self.decoder3(torch.cat([x2, x3_in], dim=1))
        x2_in = F.interpolate(x2_de, (x1.shape[2], x1.shape[3]), mode='bilinear', align_corners=True)
        x1_de = self.decoder2(torch.cat([x1, x2_in], dim=1))
        x1_in = F.interpolate(x1_de, (orig_x.shape[2], orig_x.shape[3]), mode='bilinear', align_corners=True)
        x0 = self.decoder1(torch.cat([orig_x, x1_in], dim=1))
        out = self.head(x0)
