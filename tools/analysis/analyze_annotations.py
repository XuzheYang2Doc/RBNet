...
def collect_stats(json_pattern: str = "*.json"):
    ...
    for jf in json_files:
        ...
        for s in shapes:
            ...
            # Mask or object instance count by class
            ...
            if img_w and img_h:
                ...
                # Object size relative to the full image
...
def plot_overview(stats, output_path: str = "annotation_overview.png"):
    ...
    # Panel 1: instance count per class
    ...
    axes[0].set_title("Mask count per class (training set)")

    # Panel 2: bbox width-height distribution
    if len(bbox_widths) > 0:
        ...
        axes[1].set_ylabel("Box height (pixels)")
        axes[1].set_title("Bounding box size distribution")
    else:
        axes[1].set_title("No valid bounding boxes")

    # Panel 3: normalized object center positions
    if len(center_x) > 0:
        h3 = axes[2].hist2d(
            center_x,
            center_y,
        ...
        axes[2].set_xlabel("Normalized center X (0-1)")
        axes[2].set_ylabel("Normalized center Y (0-1)")
        axes[2].set_title("Object center distribution")
    else:
        axes[2].set_title("No center data")

    # Panel 4: object size relative to image size
    if len(rel_w) > 0:
        h4 = axes[3].hist2d(
            rel_w,
            rel_h,
        ...
        axes[3].set_xlabel("Width / image width")
        axes[3].set_ylabel("Height / image height")
        axes[3].set_title("Relative object size distribution")
    else:
        axes[3].set_title("No object size data")

    fig.suptitle("Training annotation statistics", fontsize=16)
...
def plot_labels_correlogram(stats, output_path: str = "labels_correlogram.png"):
    ...
    # Count label co-occurrence within each image
...
def main():
    # Analyze LabelMe JSON files in the current directory by default
    stats = collect_stats("*.json")

    print("Number of JSON annotation files: ", stats["num_json_files"])
    print("Mask count by class:")
    ...
    print("Generated: annotation_overview.png, labels_correlogram.png")
